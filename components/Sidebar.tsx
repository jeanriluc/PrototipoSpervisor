  ...
  return (
    <aside className="w-72 flex flex-col py-8 px-6 bg-[#FDFCF9]">
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-4 px-4 py-4 rounded-3xl transition-all duration-300 group ${
                isActive 
                  ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100' 
                  : 'text-slate-500 hover:bg-orange-100/40 hover:text-slate-800'
              }`}
            >
      ...
      <div className="mt-auto px-4 py-6 bg-orange-100/30 rounded-[2rem] border border-orange-100/50">
        ...
        <button className="w-full text-center text-xs font-bold text-indigo-600 hover:text-indigo-800 transition-colors uppercase tracking-widest">
          Logout
        </button>
      </div>
    </aside>
