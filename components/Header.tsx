        ...
        <div className="relative" ref={dropdownRef}>
          <button 
            ...
            className={`relative p-2.5 rounded-2xl transition-all duration-300 ${
              showNotifications ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-slate-50 text-slate-400 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-slate-100'
            }`}
          >
            <Bell size={20} />
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 bg-rose-500 text-white text-[10px] font-black rounded-full border-2 border-white flex items-center justify-center animate-bounce">
                {unreadCount}
              </span>
            )}
          </button>
            ...
            <div className="absolute top-full right-0 mt-4 w-96 bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-orange-50/10">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">You have {unreadCount} unread alerts</p>
                </div>
                <button 
                  onClick={onClear}
                ...
                {notifications.length > 0 ? (
                  notifications.map((n) => (
                    <div 
                      ...
                      className={`p-6 border-b border-slate-50 last:border-none cursor-pointer transition-colors relative group ${
                        n.isRead ? 'bg-white opacity-60' : 'bg-indigo-50/30'
                      }`}
                    >
                      {!n.isRead && (
                      ...
                      <div className="flex gap-4">
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${
                          n.type === 'alert' ? 'bg-rose-50' : n.type === 'success' ? 'bg-emerald-50' : 'bg-indigo-50'
                        }`}>
                          {getNotifIcon(n.type)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-2 mb-1">
                            ...
                            <span className="text-[9px] font-black text-slate-400 uppercase whitespace-nowrap">{getTimeAgo(n.timestamp)}</span>
                          </div>
